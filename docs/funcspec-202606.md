...
**MapEditor** は、複数の GeoJSON ファイル（GeoReferencer 等で生成された地域別ファイル）と Excel(.xlsx) 形式の GPS ポイントデータを取り込み、国土地理院地図（地理院タイル）上で**ポイント／ルート／スポット／通行止め・通行困難場所／エリア**を編集し、GeoJSON として再出力するブラウザ単体アプリケーションです。
...
- 通行止め・通行困難場所の登録、位置移動、区分・登録理由・備考の編集（専用ファイルで入出力）
...
| 地図タイル | 国土地理院地図（標準タイル） |
| 外部 API | 国土地理院 標高 API（通行止め・通行困難場所の標高取得） |
| 起動方法 | `index.html` をブラウザで開く（ES6 モジュール読み込みのためローカルサーバ経由を推奨） |

### 1.3 関連ドキュメント

- [GeoJSON データ仕様書](dataspec-geojson-202606.md)
- [GeoJSON データ仕様書（通行止め・通行困難場所）](dataspec-geojson-closure-202606.md)
- 利用者の手引: [UsersGuide-202606.md](UsersGuide-202606.md)
...
| `closure` | 通行禁止・困難場所の指定 | 地点選択、追加・移動・削除、区分／登録理由／備考編集、専用ファイル入出力 |
| `area` | エリアの位置編集 | エリア選択、頂点編集、追加・移動・削除 |

> モード値はラジオボタンの内部値です。`closure` モードの表示名はモード切り替え欄では「通行禁止・困難場所の指定」、編集パネルの見出しでは「通行止め・通行困難場所情報」です。
...
> 通行止め・通行困難場所（`type: "closure"`）はこの統計には含まれません（専用パネルの「登録地点数」で管理します。[6 章](#6-通行禁止困難場所の指定モードclosure)参照）。
...
> 通行止め・通行困難場所（`type: "closure"`）はこのモーダルの対象外であり、統合 GeoJSON の読み込みでは**取り込まれません**（ファイル内に含まれていても無視されます）。closure データは「通行禁止・困難場所の指定」モードの専用「ファイル読み込み」で取り込みます。
...
例: `MapGPS-20260621_P15_R5_S8.geojson`

#### 3.4.2 出力時の変換処理

1. 内部の `type: "route_waypoint"` Point、`type: "route"` LineString、および `type: "closure"` Point をすべて除外
...
> **補足（通行止め・通行困難場所の分離）**: 通行止め・通行困難場所（`type: "closure"`）は統合 GeoJSON の入出力には**一切含まれません**。読み込み時は本モードの「ファイル(GeoJSON)の読み込み」では取り込まれず（[3.3.1 節](#331-読み込み種別選択モーダル)）、出力時も MapGPS には書き出されません。closure は「通行禁止・困難場所の指定」モードの専用「ファイル読み込み」「ファイル出力」で個別に扱います（[6.8 節](#68-通行止め通行困難場所のファイル入出力)参照）。
...
データ仕様の詳細は [GeoJSON データ仕様書](dataspec-geojson-202606.md) を参照。
...
## 6. 通行禁止・困難場所の指定モード（`closure`）

登山道などで通行止め・通行困難となっている地点（以下「登録地点」）を登録・編集します。データは統合 GeoJSON とは独立した専用ファイルで入出力します。

### 6.1 編集パネル

| UI 要素 | 説明 |
|--------|------|
| 登録地点数（読み取り専用） | 登録地点（`type: "closure"`）の件数 |
| 登録地点（ドロップダウン） | 登録地点の名称一覧 |
| 選択地点（テキスト入力） | 選択中地点の名称（編集可、フォーカス離脱時に保存） |
| 備考（テキスト入力） | 選択中地点の備考（編集可、フォーカス離脱時に保存） |
| 区分（ラジオ） | 「通行止め」(`closed`) / 「通行困難」(`difficult`)。変更時に即保存し、マーカー形状を更新 |
| 登録理由（ラジオ） | 「工事」/「倒木」/「落石」。変更時に即保存 |
| 追加・移動 ボタン | 地点の追加・移動モードの開始／解除 |
| 削除 ボタン | 選択中地点の削除 |
| ファイル読み込み | closure 専用 GeoJSON の読み込み |
| ファイル出力 | closure 専用 GeoJSON の出力 |

### 6.2 地図上の表示（マーカー）

登録地点は赤色 `#e60000`（12×12px）のマーカーで表示します。形状は区分（`kind`）で変化します。
...
選択中の地点はアクア色 `#00ffff` でハイライトされます。マーカーのポップアップには「名称」と「区分（登録理由）」を表示します。

### 6.3 地点の選択

ドロップダウン選択または地図上のマーカークリックで選択します。選択すると、名称・備考・区分・登録理由が各入力欄に反映され、マーカーがアクア色になります。
...
| 操作 | 結果 |
|------|------|
| 地図クリック | クリック位置に新規地点を追加。名称は `地点{連番}`、ID は `C-{連番2桁}`、区分は「通行止め」(`closed`) を初期値として付与 |
| 地点のドラッグ | 任意の地点を直接掴んで移動（あらかじめ選択しておく必要はない） |
| ボタン再クリック | モード解除（全地点のドラッグを無効化） |

- **標高の自動付与**: 新規追加時およびドラッグ移動完了時に、国土地理院標高 API から標高を取得し、座標の 3 番目の要素として付与します（取得成功時にトースト通知）。
- 新規追加時は自動的に選択状態となります。

### 6.5 名称・備考の編集

選択地点・備考のテキスト入力欄を編集し、フォーカスを外すと当該地点に保存されます。名称変更はドロップダウンの表示にも反映されます。

### 6.6 区分・登録理由の編集

区分（通行止め／通行困難）・登録理由（工事／倒木／落石）のラジオボタンを選択すると、即座に当該地点へ保存されます。区分の変更はマーカー形状とポップアップにも反映されます。

### 6.7 地点の削除

「**削除**」ボタンクリック時、確認ダイアログ「地点「`{name}`」を削除しますか？」を表示。承認時、当該地点 Feature・マーカー・ドロップダウンエントリを削除し、選択状態をクリアします。

### 6.8 通行止め・通行困難場所のファイル入出力

統合 GeoJSON とは独立した専用ファイルとして入出力します。

#### 6.8.1 ファイル読み込み

- `.geojson` を**複数同時に**選択可能（読み込み種別選択モーダルは表示されません）。
- 各ファイルから `type: "closure"` かつ Point の Feature のみを取り込みます。
- **同一 ID 重複の除外**: 既存・バッチ内と同一 `id` の地点はスキップ（スキップ件数を通知）。
- **正規化**: `id` が無ければ `C-{連番2桁}` で自動採番、`kind` が `closed`/`difficult` 以外なら未選択（？）扱い、`status` は常に `draft`。

#### 6.8.2 ファイル出力

- ファイル名: `Closure-{YYYYMMDD}_N{件数}.geojson`（例: `Closure-20260621_N3.geojson`）
- FeatureCollection のトップレベルに出力日時 `updatedAt` を付与。
- 各 Feature は `type` → `id` → `name` → `kind` →（`reason`）→ `status` →（`note`）→（`relatedRoute`）→ `updatedAt` の順に整形。未選択の区分は `unknown`、`status` は常に `draft` として出力。座標は小数点以下 5 桁に丸める。

データ仕様の詳細は [GeoJSON データ仕様書（通行止め・通行困難場所）](dataspec-geojson-closure-202606.md) を参照。
...
| `spot` | Point | スポット |
| `closure` | Point | 通行止め・通行困難場所（専用ファイルで入出力。統計には非集計） |
| `area` | Polygon | エリア |
...
| `closureMarkerMap` | closure Feature 参照 | Leaflet マーカー | 通行止め・通行困難場所編集時のマーカー検索 |
...
| `closure` | 区分依存（×／三角形／？） | 赤 `#e60000` | 12×12px |
...
| `js/elevation.js` | 国土地理院標高 API による標高取得 |
| `js/fileIO.js` | GeoJSON の読み込み／出力、Excel 読み込みの取り込み処理、closure 専用入出力 |
| `js/routeEditor.js` | ルート編集（中間点の追加・移動・削除、最適化、ハイライト） |
| `js/spotEditor.js` | スポット編集（追加・移動・削除、重複抽出） |
| `js/closureEditor.js` | 通行止め・通行困難場所編集（追加・移動・削除、区分／登録理由／備考、標高付与） |
...
- 通行止め・通行困難場所の `status` は現状、読み込み・出力ともに常に `draft`（テスト中）に固定されます
- 通行止め・通行困難場所の標高取得には国土地理院標高 API への接続（オンライン環境）が必要です
...
- [GeoJSON データ仕様書](dataspec-geojson-202606.md): 統合 GeoJSON 入出力ファイルフォーマットの詳細
- [GeoJSON データ仕様書（通行止め・通行困難場所）](dataspec-geojson-closure-202606.md): closure 専用ファイルフォーマットの詳細
- [利用者の手引](UsersGuide-202606.md): 利用者向け操作手順
...
**変更履歴**:
- v1.1 (2026-06-21): 「通行禁止・困難場所の指定」モード（`closure`）の機能を追加（[6 章](#6-通行禁止困難場所の指定モードclosure)）。通行止め・通行困難場所を統合 GeoJSON の入出力から**完全に分離**（読み込み・出力とも統合 GeoJSON には含めず、専用ファイルで個別に入出力）。スポット／通行止め・通行困難場所の「追加・移動」モードで、選択地点に限らず登録済みの全地点を直接ドラッグできる挙動を反映。関連ドキュメントを 202606 版に更新。
- v1.0 (2026-04-28): 初版。
