...
- [GeoJSON データ仕様書](dataspec-geojson-202607.md)
...
> **補足（通行止め・通行困難場所の分離）**: 通行止め・通行困難場所（`type: "closure"`）は MapEditor の入出力には**一切含まれません**。読み込み時に取り込まれず（[3.3.1 節](#331-読み込み種別選択モーダル)）、出力時も書き出されません。closure は別アプリケーション **ClosureEditor** で扱います。
...
データ仕様の詳細は [GeoJSON データ仕様書](dataspec-geojson-202607.md) を参照。
...
- [利用者の手引](UsersGuide-202607.md): 利用者向け操作手順
