...
| 内訳表示 | 「通行止め `{件数}` / 通行困難 `{件数}`」。区分未選択が 1 件以上ある場合は「/ 未選択 `{件数}`」を追記 |
| 登録地点ドロップダウン | 登録済み地点の名称一覧（名称未設定時は `名称未設定`） |
| 選択地点（テキスト入力） | 選択中地点の名称（編集可、フォーカス離脱時に保存） |
| 備考（テキスト入力） | 選択中地点の備考 `note`（編集可、フォーカス離脱時に保存） |
| 区分（ラジオ） | 通行止め（`closed`）／通行困難（`difficult`）。変更時に即保存 |
| 登録理由（ラジオ） | 工事／倒木／落石／その他。変更時に即保存。「その他」は理由なし（空値）として扱う |
...
| `kind` | `closed` / `difficult` 以外（`unknown`、未設定等）は**未選択**（空値）に正規化 |
...
例: `Closure-20260817_C3_D2.geojson`（区分未選択は件数に含めません）
...
- `kind` が未選択の地点は `"unknown"` として出力
- 座標（経度・緯度・標高）は小数点以下 5 桁に丸めて出力

#### 7.7.3 出力時の警告

区分未選択の地点が含まれる場合、「区分が未選択の地点が `{件数}` 件あります」と警告します（出力自体は実行されます）。
...
| `closure`（未選択） | `?` | 灰 `#6B7280` | 16px（当たり領域 24×24px） |

`ポイントGPS`〜`area` は `DEFAULTS.FEATURE_STYLES`、`closure` は `CLOSURE_STYLES` / `CLOSURE_ICON_BOX` / `CLOSURE_HIGHLIGHT_COLOR` に定義します。
